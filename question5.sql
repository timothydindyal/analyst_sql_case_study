SELECT YearlyRanking,
       ...
       FirstDeposit,
       TimeFirstEntry
FROM (SELECT RANK() OVER (PARTITION BY EXTRACT (YEAR FROM e.entry_week) ORDER BY SUM(e.total_entries) DESC) AS YearlyRanking, EXTRACT (YEAR FROM e.entry_week) AS Year,
      ...
      WHERE EXTRACT(YEAR FROM e.entry_week) < 2020 AND (u.first_paid_entry_date - u.registration_date) < 7
