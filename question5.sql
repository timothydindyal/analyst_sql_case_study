SELECT YearlyRanking,
       ...
       FirstDeposit,
       TimeFirstEntry
FROM (SELECT RANK() OVER (PARTITION BY Year ORDER BY Entries DESC) AS YearlyRanking,
      Year,
      EntrySport,
      Entries,
      FirstDeposit,
      TimeFirstEntry
      FROM (SELECT EXTRACT (YEAR FROM e.entry_week) AS Year,
            ...
            WHERE EXTRACT(YEAR FROM e.entry_week) < 2020 AND
                  (u.first_paid_entry_date - u.registration_date) < 7
            GROUP BY Year, EntrySport) AS year_sport_data) AS ranked_data
